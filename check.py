...
with open("output.fasta", "rU") as f1:
    lines1 = []
    for line1 in f1:
...
for i in range(len(my_read)):
    ...
    if (my_read[i] == a_read1[i] or my_read2[i] == a_read1[i] or my_read3[i] == a_read1[i]):
        count += 1
        isMatch.append(True)
    else:
